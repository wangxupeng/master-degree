...
instrisic_length=list(map(lambda x: len(x)-1,dataset_outlook))
sum_il=sum(instrisic_length)
intrinsic_infor=list(map(lambda x:x/sum_il,instrisic_length))
split_info_seperate=list(map(lambda x:-x*log(x,2),intrinsic_infor))
split_info=sum(split_info_seperate)
print("The intrinsic information is %f"%(split_info))
gain_ratio=information_gain/split_info
print("The gain ratio is %f"%(gain_ratio))
